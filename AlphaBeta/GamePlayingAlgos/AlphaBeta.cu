...
*/
__device__ 
int GPUIterativeAlphaBeta(GameState* state, int init_val){
	...
	while(state){
		...
		*/
		...
		// Find next valid move
		while(state->next_move < state->moves_length && !(state->moves[state->next_move]))
			state->next_move++;

		// If child found
		if(state->next_move < state->moves_length){
			state->next = state->makeMove(state->next_move);
...
__global__ void kernel(GameState **garr, int alpha, int beta, int depth, int isMax, int* res, int limit, unsigned int *once_done){
	...
	for(int d=2; d<=depth; d++){
		GameState *state = new TicTacToeState(*(TicTacToeState*)g);
		state->stateReset(alpha, beta, d, isMax);
		*res = GPUIterativeAlphaBeta(state, (isMax?INT_MIN:INT_MAX));	
...
*/
int cpu_alphabeta_starter(TicTacToeState *g, int depth, int isMax, int time){
	int goto_depth = 3;
	...
	*/
	...
	for(int i=0; i<num_leaves; i++){
		GameState *node;
		cudaMalloc(&node, sizeof(TicTacToeState));
		cudaMemcpy(node, children[i], sizeof(TicTacToeState), cudaMemcpyHostToDevice);
		carr[i] = node;
	}
	...
	*/
	...
	if(time>0)
		kernel<<<1+num_leaves/512, 512>>>(arr, INT_MIN, INT_MAX, depth-goto_depth, ((goto_depth%2)?!isMax:isMax), res, num_leaves, once_done);
	else
		kernel2<<<1+num_leaves/512, 512>>>(arr, INT_MIN, INT_MAX, depth-goto_depth, ((goto_depth%2)?!isMax:isMax), res, num_leaves, once_done);
